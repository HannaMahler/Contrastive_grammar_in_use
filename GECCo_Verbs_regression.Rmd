...
- the number of verb phrases per text (NR_vp)
...
+++ Side note: +++
Having text as a random effect + (1|Text_id), does not make sense, since there is only one observation for each text. So we can't really speak of a "grouping variable" since there are no groups.
...
- English probably uses more VPs than German. 
...
sd(Intercept)     0.12      0.02     0.08     0.18 1.00     3801     5527

Population-Level Effects: 
                 Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
Intercept            2.55      0.03     2.49     2.62 1.00     3327     4715
Language1            0.06      0.00     0.05     0.06 1.00    11927     8571
Mode1                0.09      0.03     0.03     0.16 1.00     3660     4517
STTR_z              -0.04      0.01    -0.05    -0.03 1.00    10423     8030
Language1:Mode1      0.01      0.00     0.00     0.02 1.00    10380     8567
Language1:STTR_z    -0.01      0.00    -0.02    -0.00 1.00     9615     8366
...
# This function shows the mean value predicted by the model with 95% credible-intervals, for each individual predictor. If not specified otherwise, the function will use the mean or reference level of the other predictors for calculating the predictions. These predictions are on the original count scale.

conditional_effects(vp_model_poisson, "STTR_z")
...
ggplot(filter(vp_model_poisson_transformed, Parameter == "b_Intercept", Iteration > 1000),
  ...
  geom_vline(xintercept = c(2.49, 2.62), ## enter values from above here
             col = "blue", linetype = 2) +
  theme_light() +
...
ggplot(filter(vp_model_poisson_transformed, Parameter == "b_Intercept", Iteration > 1000),
  ...
  geom_vline(xintercept = c(exp(2.49), exp(2.62)), ## enter values from above here (exp(value))
             col = "blue", linetype = 2) +
  theme_light() +
...
```{r visualise actual data distribution}
ggplot()+
...
ggplot(filter(vp_model_poisson_transformed, Parameter == "b_STTR_z", Iteration > 1000), 
  ...
  geom_vline(xintercept = c(-0.0546, -0.0327), col = "blue", linetype = 2)+ ## enter values from above here
  theme_light()+
...
ggplot(filter(vp_model_poisson_transformed, Parameter == "b_Mode1", Iteration > 1000), aes(x = value))+
  ...
  geom_vline(xintercept = c(0.026, 0.157), col = "blue", linetype = 2)+ ## enter values from above here
  theme_light()+
...
ggplot(filter(vp_model_poisson_transformed, Parameter == "b_Language1", Iteration > 1000), aes(x = value))+
  ...
  geom_vline(xintercept = c(0.051, 0.0637), col = "blue", linetype = 2)+ ## enter values from above here
  theme_light()+
...
# Evid.Ratio of 3999 indicates that Mode is a stronger effect than Density
```
...
#saveRDS(vp_model_1, file = "Models/vp_model_1.rds")
...
#saveRDS(vp_model_2, file = "Models/vp_model_2.rds")
...
#saveRDS(vp_model_3, file = "Models/vp_model_3.rds")
...
#saveRDS(vp_model_4, file = "Models/vp_model_4.rds")
...
#saveRDS(vp_model_5, file = "Models/vp_model_5.rds")
...
#saveRDS(vp_model_7, file = "Models/vp_model_7.rds")
