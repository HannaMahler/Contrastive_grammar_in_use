...
+++ Side note: +++
Using Text as a random effect + (1|Text_id) does not make sense, since there is only one observation for each text. So we can't really speak of a "grouping variable" since there are no groups.
...
Group-Level Effects: 
~Register (Number of levels: 14) 
              Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
sd(Intercept)     0.21      0.03     0.15     0.29 1.00     4219     5796

Population-Level Effects: 
                 Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
Intercept            0.52      0.06     0.41     0.64 1.00     3588     4424
Language1            0.57      0.01     0.56     0.59 1.00    10034     8393
Mode1               -0.18      0.06    -0.29    -0.07 1.00     3538     5021
STTR_z               0.05      0.02     0.02     0.08 1.00     8532     7988
Language1:Mode1      0.04      0.01     0.02     0.06 1.00     8955     8236
Language1:STTR_z    -0.06      0.01    -0.08    -0.03 1.00     8419     8252
...
conditional_effects(vp_nf_model_poisson, effects = "Mode:STTR_z")
conditional_effects(vp_nf_model_poisson)$Language
...
ggplot(filter(vp_nf_model_poisson_transformed, Parameter == "b_Intercept", Iteration > 1000),
  ...
  geom_vline(xintercept = c(0.405, 0.638), ## enter values from above here
             col = "blue", linetype = 2) +
  theme_light() +
...
ggplot(filter(vp_nf_model_poisson_transformed, Parameter == "b_Intercept", Iteration > 1000),
  ...
  geom_vline(xintercept = c(exp(0.405), exp(0.638)), ## enter values from above here (exp(value))
             col = "blue", linetype = 2) +
  theme_light() +
...
ggplot(filter(vp_nf_model_poisson_transformed, Parameter == "b_Language1", Iteration > 1000), aes(x = value))+
  ...
  geom_vline(xintercept = c(0.556, 0.593), col = "blue", linetype = 2)+ ## enter values from above here
  theme_light()+
...
NR_vp_nf ~ 1 + Language + Mode + STTR_z + offset(log(NR_tokens_phw)) + Language:Mode + Language:STTR_z + (1|Register)
This is the model that includes all predictors that are relevant from a theoretical perspective. There are, however, other possible models for this data set. For the sake of completeness, I am fitting these models below and comparing them to the theoretically informed model.
...
vp_nf_model_1 <- brm(
  NR_vp_nf ~ 1 + offset(log(NR_tokens_phw)) + Language,
  data = texts,
  family = poisson(),
  prior = priors_1,
  warmup = 2000,
...
#saveRDS(vp_nf_model_1, file = "Models/vp_nf_model_1.rds")
#vp_nf_model_1 <- read_rds("Models/vp_nf_model_1.rds")

#summary(vp_nf_model_1)
#plot(vp_nf_model_1)
```
...
vp_nf_model_2 <- brm(
  NR_vp_nf ~ 1 + offset(log(NR_tokens_phw)) + Language + Mode,
  data = texts,
  family = poisson(),
  prior = priors_2,
  warmup = 2000,
...
#saveRDS(vp_nf_model_2, file = "Models/vp_nf_model_2.rds")
#vp_nf_model_2 <- read_rds("Models/vp_nf_model_2.rds")

#summary(vp_nf_model_2)
#plot(vp_nf_model_2)
```
...
vp_nf_model_3 <- brm(
  NR_vp_nf ~ 1 + Language + Mode + STTR_z + offset(log(NR_tokens_phw)),
  data = texts,
  family = poisson(),
  prior = priors_3,
  warmup = 2000,
...
#saveRDS(vp_nf_model_3, file = "Models/vp_nf_model_3.rds")
#vp_nf_model_3 <- read_rds("Models/vp_nf_model_3.rds")

#summary(vp_nf_model_3)
#plot(vp_nf_model_3)
```
...
vp_nf_model_4 <- brm(
  NR_vp_nf ~ 1 + Language + Mode + STTR_z + offset(log(NR_tokens_phw)) + Language:Mode,
  data = texts,
  family = poisson(),
  prior = priors_4,
  warmup = 2000,
...
#saveRDS(vp_nf_model_4, file = "Models/vp_nf_model_4.rds")
#vp_nf_model_4 <- read_rds("Models/vp_nf_model_4.rds")

#summary(vp_nf_model_4)
#plot(vp_nf_model_4)
```
...
vp_nf_model_5 <- brm(
  NR_vp_nf ~ 1 + Language + Mode + STTR_z + offset(log(NR_tokens_phw)) + Language:Mode + Language:STTR_z,
  data = texts,
  family = poisson(),
  prior = priors_5,
  warmup = 2000,
...
#saveRDS(vp_nf_model_5, file = "Models/vp_nf_model_5.rds")
#vp_nf_model_5 <- read_rds("Models/vp_nf_model_5.rds")

#summary(vp_nf_model_5)
#plot(vp_nf_model_5)
```

```{r vp nf model 6}
vp_nf_model_6 <- vp_nf_model_poisson

#summary(vp_nf_model_6)
#plot(vp_nf_model_6)
```
...
vp_nf_model_7 <- brm(
  NR_vp_nf ~ 1 + Language + Mode + STTR_z + offset(log(NR_tokens_phw)) + Language:Mode + Language:STTR_z + Register,
  data = texts,
  family = poisson(),
  prior = priors_7,
  warmup = 2000,
...
#saveRDS(vp_nf_model_7, file = "Models/vp_nf_model_7.rds")
#vp_nf_model_7 <- read_rds("Models/vp_nf_model_7.rds")

#summary(vp_nf_model_7)
#plot(vp_nf_model_7)
```
...
vp_nf_model_1: NR_vp_nf ~ 1 + offset(log(NR_tokens_phw)) + Language
vp_nf_model_2: NR_vp_nf ~ 1 + offset(log(NR_tokens_phw)) + Language + Mode
vp_nf_model_3: NR_vp_nf ~ 1 + offset(log(NR_tokens_phw)) + Language + Mode + STTR_z
vp_nf_model_4: NR_vp_nf ~ 1 + offset(log(NR_tokens_phw)) + Language + Mode + STTR_z + Language:Mode
vp_nf_model_5: NR_vp_nf ~ 1 + offset(log(NR_tokens_phw)) + Language + Mode + STTR_z + Language:Mode + Language:STTR_z
vp_nf_model_6: NR_vp_nf ~ 1 + offset(log(NR_tokens_phw)) + Language + Mode + STTR_z + Language:Mode + Language:STTR_z + (1|Register)
vp_nf_model_7: NR_vp_nf ~ 1 + offset(log(NR_tokens_phw)) + Language + Mode + STTR_z + Language:Mode + Language:STTR_z + Register

This code take a while to run!
```{r}
vp_nf_model_1_loo <- loo(vp_nf_model_1)
vp_nf_model_2_loo <- loo(vp_nf_model_2)
vp_nf_model_3_loo <- loo(vp_nf_model_3)
vp_nf_model_4_loo <- loo(vp_nf_model_4)
vp_nf_model_5_loo <- loo(vp_nf_model_5)
vp_nf_model_6_loo <- loo(vp_nf_model_6)
vp_nf_model_7_loo <- loo(vp_nf_model_7)

loos <- loo_compare(vp_nf_model_1_loo, vp_nf_model_2_loo, vp_nf_model_3_loo, 
                    vp_nf_model_4_loo, vp_nf_model_5_loo, vp_nf_model_6_loo, vp_nf_model_7_loo)
loos
```

This tells us that the model we originally built (model 6) is indeed the best model out of the set (since it was chosen as the baseline). The model that has register as a fixed effect instead of a random effect (model 7) is nearly as good. All other models without Register perform significantly worse.
